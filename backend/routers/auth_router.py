...
def register(req: RegisterRequest, db: Session = Depends(get_db)):
    ...
    # Create account
    # Students are auto-verified; teachers require admin approval before they can log in.
    student = Student(
        name       = req.name.strip(),
        ...
        is_verified = not req.is_teacher,
    )
    db.add(student)
    db.flush()
    ...
    #  TEACHER — pending admin verification, no token issued
    if req.is_teacher:
        return {
            ...
            'is_teacher':  True,
            'message': (
                f'Thanks {student.name}! Your teacher account has been created for '
                f'{enrolled_subjects[0] if enrolled_subjects else req.teach_subject_name}. '
                'It is now waiting for verification from an administrator. '
                'You will be able to log in once your account has been approved.'
            ),
        }

    #  STUDENT — registration successful, return to login (no auto-login)
    msg = (
...
def login(req: LoginRequest, db: Session = Depends(get_db)):
    ...
    # Teachers must be approved by an administrator before they can log in
    if student.is_teacher and not student.is_verified:
        raise HTTPException(
            403,
            'Your teacher account is still waiting for verification from an '
            'administrator. You will be able to log in once it has been approved.'
        )
