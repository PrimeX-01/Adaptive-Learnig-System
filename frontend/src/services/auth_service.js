...
function normalize(data) {
  ...
  return {
    ...
    user: {
      ...
      first_name,
      last_name,
      role:           data.is_admin ? 'admin' : (data.is_teacher ? 'teacher' : 'student'),
...
export async function registerUser(payload) {
  ...
  // Backend now branches on role:
  //   teacher → { status: 'pending_verification', message, ... }  — no token issued
  //   student → { status: 'registered', message, ... }            — no token issued
  // Registration never auto-logs-in; the caller routes the person to login afterward.
